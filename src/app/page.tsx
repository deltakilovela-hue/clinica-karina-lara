...
const s = {
  page: { minHeight: '100vh', background: '#FAF7F2', fontFamily: "'Lato', sans-serif" } as React.CSSProperties,
  header: { background: 'white', borderBottom: '1px solid #E8DDD0', padding: '0 24px' } as React.CSSProperties,
  headerInner: { maxWidth: '1200px', margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: '64px' } as React.CSSProperties,
  logo: { width: '36px', height: '36px', borderRadius: '50%', background: 'linear-gradient(135deg, #7B1B2A, #A63244)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: '13px', fontWeight: '700', fontFamily: "'Playfair Display', serif" } as React.CSSProperties,
  main: { maxWidth: '1200px', margin: '0 auto', padding: '40px 24px' } as React.CSSProperties,
  card: { background: 'white', borderRadius: '16px', border: '1px solid #E8DDD0', padding: '24px' } as React.CSSProperties,
  statCard: { background: 'white', borderRadius: '14px', border: '1px solid #E8DDD0', padding: '20px 24px' } as React.CSSProperties,
  moduleCard: { background: 'white', borderRadius: '14px', border: '1px solid #E8DDD0', padding: '24px', display: 'block', textDecoration: 'none', transition: 'all 0.2s' } as React.CSSProperties,
}
...
export default function DashboardPage() {
  ...
  if (cargando) return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#FAF7F2' }}>
      <div style={{ textAlign: 'center' }}>
        <div style={{ width: '40px', height: '40px', border: '3px solid #E8DDD0', borderTopColor: '#7B1B2A', borderRadius: '50%', animation: 'spin 0.8s linear infinite', margin: '0 auto 16px' }} />
        <p style={{ color: '#9B7B65', fontSize: '14px', fontFamily: "'Lato', sans-serif" }}>Cargando...</p>
      </div>
    </div>
  )
  ...
  const modulos = [
    ...
    { nombre: 'Antropometría', desc: 'Peso, talla y percentiles OMS', href: '#', icono: '📏', activo: false },
      ...
      <header style={s.header}>
        <div style={s.headerInner}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            <div style={s.logo}>KL</div>
            <div>
              <p style={{ fontFamily: "'Playfair Display', serif", fontWeight: '600', fontSize: '16px', color: '#2C1810' }}>Clínica Karina Lara</p>
              <p style={{ fontSize: '11px', color: '#8B6914', letterSpacing: '0.3px' }}>Nutrición Clínica Especializada</p>
            </div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
            <p style={{ fontSize: '13px', color: '#9B7B65', display: 'none' }}>{hoy}</p>
            ...
            <button onClick={cerrarSesion} style={{
              ...
              color: '#6B4F3A', cursor: 'pointer', fontFamily: "'Lato', sans-serif"
            }}>Salir</button>
          </div>
        </div>
      </header>
      ...
      <main style={s.main}>
        ...
        <div style={{ marginBottom: '32px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '14px' }}>
            {modulos.map(mod => (
              <Link key={mod.nombre} href={mod.activo ? mod.href : '#'}
                style={{ ...s.moduleCard, opacity: mod.activo ? 1 : 0.5, cursor: mod.activo ? 'pointer' : 'not-allowed' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' }}>
                  <span style={{ fontSize: '24px' }}>{mod.icono}</span>
                  <span style={{
                    fontSize: '11px', padding: '3px 10px', borderRadius: '20px', fontWeight: '600',
                    background: mod.activo ? '#D8F3DC' : '#F2EDE4',
                    color: mod.activo ? '#2D6A4F' : '#9B7B65'
                  }}>{mod.activo ? 'Activo' : 'Próximo'}</span>
                </div>
        ...
        {/* Pacientes recientes */}
        {pacientes.length === 0 ? (
          <div style={{ ...s.card, textAlign: 'center', padding: '48px', border: '2px dashed #E8DDD0' }}>
            ...
            <div style={{ ...s.card, padding: 0, overflow: 'hidden' }}>
              {pacientes.slice(0, 5).map((p, i) => (
                <Link key={p.id} href={`/pacientes/${p.id}`} style={{
                  ...
                  borderBottom: i < Math.min(pacientes.length, 5) - 1 ? '1px solid #F2EDE4' : 'none',
                  background: 'white'
                }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
                    <div style={{
                      ...
                      fontSize: '16px', fontWeight: '700', fontFamily: "'Playfair Display', serif"
                  ...
                  <span style={{
                    ...
                    background: '#F2EDE4', color: '#6B4F3A', textTransform: 'capitalize', fontWeight: '500'
                  }}>{p.sexo}</span>
                </Link>
              ))}
            </div>
